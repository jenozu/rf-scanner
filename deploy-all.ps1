...
ssh -o BatchMode=yes $vpsHost "cd ${remotePath}/server; npm install; pm2 describe rf-api > /dev/null 2>&1 && pm2 restart rf-api || pm2 start ecosystem.config.cjs && pm2 save"
